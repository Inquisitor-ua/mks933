...
def parse_films(html):
    ...
    names = []
    for film in films:
        name = film.find('a', class_='short-title').text
        names.append(name)
    return names

def get_random_films():
    page = random.randint(1, 343)
    url = f'https://kinoukr.tv/films/page/{page}'
    html = make_html(url)
